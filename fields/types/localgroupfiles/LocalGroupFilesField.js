...
var Item = React.createClass({
	...
	render: function () {
		...
		return <div className={itemClassName} data-id={_id} key={this.props.key} style={itemStyle}>{body}</div>;
	}
...
module.exports = Field.create({
	...
	reorderFiles: function() {
		let postId = location.href.substr(location.href.lastIndexOf('/') + 1)
		let items = document.querySelectorAll('.file-item')
		const orderedItems = [];
		for(let item of items) {
			let i = _.findWhere(images, {_id: item.getAttribute('data-id')})
			orderedItems.push(i);
			delete i.parentId
			delete i.toggleDelete
		}
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "http://localhost:3011/post/"+postId, true);
		xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
		xhr.onload = function() {
			if(xhr.status == 200) {
				document.querySelector('.btn-reorder').classList.toggle('btn-info')
				document.querySelector('.btn-reorder').classList.toggle('btn-success')
		...
		xhr.send(JSON.stringify({images: orderedItems}));
	},
	...
	renderToolbar: function () {
		...
		return (
			<div className='files-toolbar row col-sm-6 col-md-12'>
				<div className='pull-left'>
					<button type='button' className='btn btn-default btn-upload' onClick={this.changeFiles}>Upload</button>
					<button type='button' className='btn btn-info btn-reorder' onClick={this.reorderFiles}>Reorder</button>
					{clearFilesButton}
				</div>
			</div>
		);
	...
	renderPlaceholder: function () {
		return (
			<div className='file-field file-upload row col-sm-6 col-md-12' onClick={this.changeFiles}>
	...
	componentDidMount: function() {
		Dragula([document.querySelector('.files-container')], {
			direction: 'vertical'
		})
		.on('drag', function() {
