...
function groupimages(list, path, options) {
	...
	if (options.initial) {
		throw new Error('Invalid Configuration\n\n' +
			'groupimages fields (' + list.key + '.' + path + ') do not currently support being used as initial fields.\n');
	}
	if (options.overwrite !== false) {
		options.overwrite = true
	}

	groupimages.super_.call(this, list, path, options);

	// validate destination dir
	if (!options.dest) {
		throw new Error('Invalid Configuration\n\n' +
			'groupimages fields (' + list.key + '.' + path + ') require the "dest" option to be set.');
	}
...
util.inherits(groupimages, super_);
...
groupimages.prototype.addToSchema = function() {
...
groupimages.prototype.format = function(item, i) {
...
groupimages.prototype.hasFormatter = function() {
...
groupimages.prototype.href = function(file) {
...
groupimages.prototype.isModified = function(item) {
...
groupimages.prototype.validateInput = function(data) {//eslint-disable-line no-unused-vars
...
groupimages.prototype.updateItem = function(item, data) {//eslint-disable-line no-unused-vars
...
groupimages.prototype.uploadFiles = function(item, files, update, callback) {
...
groupimages.prototype.getRequestHandler = function(item, req, paths, callback) {
...
groupimages.prototype.handleRequest = function(item, req, paths, callback) {
...
exports = module.exports = groupimages;
